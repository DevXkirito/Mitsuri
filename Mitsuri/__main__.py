...
from Mitsuri.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("Mitsuri.modules." + module_name)
...
def error_handler(update, context):
    ...
    context.bot.send_message(chat_id=-1001483575735, text=message, parse_mode=ParseMode.HTML)
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.send_photo(f"@{SUPPORT_CHAT}", Mitsuri_IMG,
             reply_markup=InlineKeyboardMarkup(
                [
                  [                  
                       InlineKeyboardButton(
    ...
    if WEBHOOK:
        ...
        LOGGER.info(f"Mitsuri deployed. | BOT: [@MitsuriXRobot]")
